...
# Load data
data <- read.csv("DataOutputs/anovaCORE-DE-significant-BH.csv", header = TRUE)
...
colors <- c("#F6BFFF", "#FF7FEB", "#3F0C00", "#FFB4C3", "#BBFF3F", "#D40059", "#FF261F", "#9C2AFF", "#480AFF", "#C0C0C0", "#61C5BA", "#35FF35", "#00D7FF", "#FFAC35", "#CA5823", "#FFE60A", "#006A17", "#9C00BF")
KOcolors <- list()
...
# Split the tree into Light and Dark sections, where 1 = Day, 2 = Night
SplitTree <- cutree(CHMov_gene_hclust, k =2) %>%
  as.data.frame()

write.csv(SplitTree, file = "DataOutputs/anovaCORE-DE-sig-DL.csv", row.names = T)
